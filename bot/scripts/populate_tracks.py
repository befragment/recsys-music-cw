...
def get_mp3_metadata(file_path: Path) -> Optional[dict]:
    ...
    try:
        ...
        if audio.tags:
            # ID3 теги
            title = (
                str(audio.tags.get("TIT2", [""])[0]) if audio.tags.get("TIT2") else None
            )
            artist = (
                str(audio.tags.get("TPE1", [""])[0]) if audio.tags.get("TPE1") else None
            )
            album = (
                str(audio.tags.get("TALB", [""])[0]) if audio.tags.get("TALB") else None
            )
...
async def populate_tracks_table(
    data_dir: Path, batch_size: int = 100, skip_if_exists: bool = False
):
    """
    ...
    async with async_session_factory() as session:
        ...
        if existing:
            if skip_if_exists:
                logger.info(
                    "Таблица tracks уже содержит данные. Пропускаем заполнение."
                )
                return
            else:
        ...
        for i, mp3_path in enumerate(mp3_files, 1):
            ...
            if len(tracks_to_insert) >= batch_size:
                ...
                await session.commit()
                logger.info(
                    f"Обработано {i}/{len(mp3_files)} файлов (успешно: {success_count}, ошибок: {error_count})"
                )
                tracks_to_insert = []
        ...
        logger.success(
            f"Завершено! Успешно добавлено: {success_count}, ошибок: {error_count}"
        )
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Заполнение таблицы tracks из MP3 файлов"
    )
    parser.add_argument(
        "--skip-if-exists",
        action="store_true",
    ...
    parser.add_argument(
        "--data-dir", type=str, default=None, help="Путь к директории с MP3 файлами"
    )
