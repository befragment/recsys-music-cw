...
def upgrade() -> None:
    ...
    op.create_table('interactions',
        ...
        sa.Column('action', postgresql.ENUM('like', 'dislike', 'skip', 'listen', name='interaction_action'), nullable=False),
        sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=False),
    ...
    op.create_index('ix_interactions_user_track_action', 'interactions', ['user_id', 'track_id', 'action'], unique=False)
    
    # Create user_feature_weights table
    op.create_table('user_feature_weights',
        sa.Column('user_id', sa.Integer(), nullable=False),
